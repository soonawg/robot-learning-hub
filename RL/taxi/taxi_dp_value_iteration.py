"""
Taxi-v3 환경에서의 Value Iteration 구현
이 코드는 강화학습의 동적 프로그래밍 방법 중 하나인 Value Iteration을 구현합니다.

Value Iteration은 다음의 단계로 진행됩니다:
1. 모든 상태의 가치를 0으로 초기화
2. 각 상태에서 모든 행동에 대한 기대 가치를 계산
3. 각 상태의 가치를 최대 기대 가치로 업데이트
4. 변화량이 임계값보다 작아질 때까지 반복
5. 최종 가치를 기반으로 최적 정책 도출
"""
