...
# # 더 세밀한 구간 분할과 안전한 범위 설정
# bins = [np.linspace(-2.4, 2.4, 10),      # cart position (실제 범위에 맞게 조정)
#         np.linspace(-3.0, 3.0, 10),      # cart velocity  
#         np.linspace(-0.2, 0.2, 10),      # pole angle (실제 범위에 맞게 조정)
...
#         # digitize 결과를 0~9 범위로 제한
#         discrete_idx = np.clip(np.digitize(clipped_val, bins[i]) - 1, 0, 9)
...
#         # 리워드 엔지니어링 (선택적)
#         if terminated and steps < 195:  # 일찍 끝나면 페널티
#             reward = -100
...
#         print(f"에피소드 {episode + 1}: 평균 리워드={avg_reward:.2f}, 평균 길이={avg_length:.2f}, 입실론={epsilon:.3f}")
...
#     print(f"테스트 에피소드 {test_episode + 1}: {steps}스텝, 총 리워드: {total_reward}")
